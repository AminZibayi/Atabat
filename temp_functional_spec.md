...
# EXECUTED/DONE Implementation Plan - Atabat Management App

This plan outlines the scaffolding for the Atabat Management App, replacing the legacy WebForms site with a modern Next.js + Payload CMS application. It includes a Playwright-based scraper engine for backend integration.

## User Review Required

> [!IMPORTANT] > **Scraping dependency**: The app relies on `playwright` to interact with the legacy site. Ensure the hosting environment supports running a browser (e.g., using a Dockerfile with browser dependencies).

>

> **Captcha Solving**: The plan includes a `CaptchaSolver` interface. You must implement the actual OCR or manual solving logic (e.g., calling an external OCR API) as "reads captcha" is a complex task depending on the captcha type.

>

> **Auth Persistence**: The prompt requests saving cookies to a file. In serverless/containerized environments, local files may be lost. I will implement file-based storage, but consider using Redis or the Database for production.

## Proposed Changes

### 1. Dependencies and Configuration

Ensure `playwright` is available for production use.

#### [MODIFY] [package.json](file:///d:/Projects/Atabat/AppV2/package.json)

- Move `playwright` to `dependencies`.

### 2. Payload CMS Structure (Database)

Define data models for Settings (Credentials, OTP) and Application Data.

#### [NEW] [Global: Settings](file:///d:/Projects/Atabat/AppV2/src/globals/Settings.ts)

- Fields: `kargozarUsername`, `kargozarPassword`, `dailyOtp` (updated manually), `captchaApiKey` (optional).

#### [NEW] [Collection: Reservations](file:///d:/Projects/Atabat/AppV2/src/collections/Reservations.ts)

- Fields: `status` (Pending, Reserved, Paid), `nationalId`, `tripId`, `passengerDetails`, `receiptData`, `paymentUrl`.

#### [MODIFY] [payload.config.ts](file:///d:/Projects/Atabat/AppV2/src/payload.config.ts)

- Register `Settings` global and `Reservations` collection.

### 3. Scraper Engine

Implement the core logic to interact with the legacy website.

#### [NEW] [Scraper: Client](file:///d:/Projects/Atabat/AppV2/src/lib/scraper/index.ts)

- Class `AtabatScraper`.
- Methods: `login()`, `searchTrips(query)`, `reserve(passengerData)`, `getReceipt(resId)`.
- Handles Cookie persistence (reading/writing to `auth_cookies.json` or similar).

### 4. API Layer

Expose scraper functionality to the frontend.

#### [NEW] [API: Search](<file:///d:/Projects/Atabat/AppV2/src/app/(frontend)/api/search/route.ts>)

- `GET /api/search`: Calls `scraper.searchTrips`.

#### [NEW] [API: Reserve](<file:///d:/Projects/Atabat/AppV2/src/app/(frontend)/api/reserve/route.ts>)

- `POST /api/reserve`: Calls `scraper.reserve`.

### 5. Frontend UI

Modern, responsive UI for the application.

#### [NEW] [Page: Home/Search](<file:///d:/Projects/Atabat/AppV2/src/app/(frontend)/page.tsx>)

- Search form (Date range, Province, etc.).
- Results table (Client-side fetching from `/api/search`).

#### [NEW] [Page: Reservation](<file:///d:/Projects/Atabat/AppV2/src/app/(frontend)/reservation/[id]/page.tsx>)
...
- Run `pnpm test:e2e` (if existing tests cover it).
- I will verify the scraper logic by mocking the target site or running against the live site if allowed (CAUTION: Running against live government/org sites should be done carefully). _I will focus on verification of the code structure and "Safe" execution paths._

### Manual Verification

1.   **Admin Panel**: Verify ability to enter "OTP" and "Credentials" in Payload Admin.
2.   **Search**: Verify the API returns JSON structure matching the legacy table.
3.   **Flow**: Verify the sequence of Login -> Scrape -> Return Data.
